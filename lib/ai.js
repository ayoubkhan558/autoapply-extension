...
const AA_RESUME_PROMPT = [
  ...
  "",
  "personal: firstName, lastName, middleName, preferredName, email, phone, address1, address2, city, state, zip, country, dateOfBirth, gender, nationality, pronouns",
  "links: linkedin, github, portfolio, website, twitter, stackoverflow, dribbble, behance, medium",
  "professional: currentTitle, currentCompany, experienceYears, currentSalary, desiredSalary, noticePeriod, availableStartDate, workAuthorization, needsSponsorship, willingToRelocate, remotePreference, securityClearance, skills, linkedinHeadline, references, coverLetter",
...
async function aaOpenAIJson(opts, systemText, userText) {
  ...
  if (opts.provider === "groq") body.response_format = { type: "json_object" };
...
async function aaListGemini(opts) {
  ...
  const url = "https://generativelanguage.googleapis.com/v1beta/models?pageSize=200&key=" + encodeURIComponent(opts.apiKey);
  const res = await fetch(url);
...
function aaBuildAnswerPrompt(profile, questions, jobText) {
  ...
  return [
    ...
    "If the profile lacks detail for a question, give a reasonable answer in the candidate's voice.",
    "",
    "CANDIDATE PROFILE (JSON):",
    profStr,
    "",
